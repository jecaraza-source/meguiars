...
- **Catálogo y precios:** `services` es de la organización y `service_center_config` define la disponibilidad y los valores propios por centro (ADR 0009). La OS debe **copiar** precio, costo, duración y motor a su línea (`freezeServiceLine`) y nunca releerlos del catálogo. El margen estándar se calcula sólo con `standardMargin`. Los servicios se desactivan, no se borran.
- **Agenda y horas:** guarda instantes en UTC y captura o muestra con la zona del centro (`zonedToUtc`/`utcToZoned` de `packages/domain/src/agenda/zoned-time.ts`; "hoy" = `todayIn(center.timezone)`). Los conflictos de recursos se resuelven en la base (exclusion constraints), no en la UI. Un override sólo con `agenda.manage`, motivo y evento auditado (ADR 0010).
- **Orden de Servicio:** totales, niveles de descuento y reglas por canal se deciden en la base (`private.recalc_service_order`); el dominio sólo los refleja para vistas previas. Toda edición de una OS envía su `version` (concurrencia optimista, `40001`) y las líneas conservan el precio congelado (ADR 0011).
- **Ejecución y evidencias:** las fotos van al bucket privado `service-order-evidence` en la ruta `evidencePath` (`<org>/<centro>/<OS>/<uuid>.<ext>`), redimensionadas en el cliente (`targetImageSize`, JPEG 0.7), y se leen sólo con URL firmadas. Las políticas de `storage.objects` usan las mismas funciones que la RLS. El tiempo real lo calcula la base; el consumo sólo aplica a insumos configurados y congela estándar y costo. No construyas existencias ni almacenes (ADR 0012).
- **Membresías:** la membresía guarda las condiciones del plan adquirido (no las releas del plan); el estado efectivo sale de `private.membership_status` / `membershipStatus` y los periodos de `membershipPeriod`. Una redención es un descuento de la OS con `source = 'membresia'` y `request_id`; no la simules con descuentos manuales. KPIs (MRR y demás) sólo con `@meguiars/analytics` (ADR 0013).
- **CRM:** el consentimiento por canal vive en `contact_preferences` y sólo se escribe con `private.put_contact_preference` (sincronizado con `clients.marketing_*`); nunca crees seguimientos por canales sin consentimiento ni envíes mensajes: `crm_tasks` es una cola. Segmentos, próxima visita y valor acumulado se derivan de OS y membresías al leer (`private.crm_rows` ↔ `customerSegment`/`nextVisitState`/`CRM_RULES`); no guardes contadores (ADR 0014).
